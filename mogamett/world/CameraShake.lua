local CameraShake = {
    cameraShakeInit = function(self)
        self.uid = 0
        self.p = self.mm.Vector(self.camera:pos())
        self.v = self.mm.Vector(0, 0)
        self.a = self.mm.Vector(0, 0)
        self.shakes = {}
    ...
    cameraShakeUpdate = function(self, dt)
        ...
        self.v = self.mm.Vector(self.mm.utils.math.random(-self.shake_intensity, self.shake_intensity), self.mm.utils.math.random(-self.shake_intensity, self.shake_intensity))
    ...
    cameraShakeAddPos = function(self, x, y, intensity, duration)
        local d = self.mm.Vector.distance(Vector(x, y), Vector(self.player.x, self.player.y))
