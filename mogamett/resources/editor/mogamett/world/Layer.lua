...
function Layer:addShader(name, vertex_path, fragment_path)
    ...
    self.canvases[name] = love.graphics.newCanvas(mg.screen_width, mg.screen_height)
                ...
                if i == 1 then
                    self.canvases[name]:renderTo(function()
                        ...
                        for _, entity in ipairs(self.entities) do 
                            if utils.table.contains(self.shader_classes[name], entity.class_name) then entity:draw() end
                        end
                    ...
                    self.main_canvas:renderTo(function()
                        self.world:renderAttach()
                        for _, entity in ipairs(self.entities) do 
                            if not utils.table.contains(self.shader_classes[name], entity.class_name) then entity:draw() end
                        end
                        self.world:renderDetach()
                    end)
